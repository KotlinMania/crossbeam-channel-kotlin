# Claude Code Project Instructions — crossbeam-channel-kotlin

## Project Overview

This is **crossbeam-channel-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`crossbeam-channel`](https://crates.io/crates/crossbeam-channel).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/crossbeam-channel/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/crossbeam-channel>
